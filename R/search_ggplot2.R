...
#' @export
#' @examples
#' search_ggplot2(2015:2019, "phenology", "Nature")
search_ggplot2 <- function(year, search_term, publication=NULL) {
  # ... function body ...
}
...
search_ggplot2 <- function(year, search_term, publication=NULL) {
  ...
  p=ggplot(dt,aes(x=year, y=no_papers))+
    ...
    labs(x="Year",y="Number of papers",title=paste("The Rise of the [","] in [", publication,"]"))+
  ...
  plot_filename <- paste0("plot_", paste(min(year),max(year), collapse = ":"), publication, ".tiff")
